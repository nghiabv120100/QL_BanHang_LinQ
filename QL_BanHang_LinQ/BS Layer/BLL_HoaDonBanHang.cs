...
namespace QL_BanHang_LinQ.BS_Layer
{
    public class BLL_HoaDonBanHang
    {
        ...
        public static List<HoaDon> LayToanBoDanhSachHoaDon()
        {
            ...
            return Query_DAL.LayToanBoDanhSachHoaDon();
        }
        ...
        // Return true nếu đã tồn tại mã hoá đơn
        private static bool CheckKey(string MaHD)
        {
            List<HoaDon> dsHD = LayToanBoDanhSachHoaDon();
            foreach(HoaDon hd in dsHD)
            {
                if (hd.MaHoaDon.Trim() == MaHD)
                    return true;
            }
            return false;
        }
